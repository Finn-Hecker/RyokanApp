<script lang="ts">
  ...
  function onOpenCreate() { currentView.set('create'); }
...
<div class="h-full flex overflow-hidden">
  ...
  <div class="lg:hidden">
    <Sidebar isOpen={isMenuOpen} close={() => (isMenuOpen = false)} alwaysVisible={false} />
  </div>
    ...
    <div class="max-w-7xl mx-auto w-full px-8">
      ...
      <div class="flex items-center justify-between pt-6 mb-8">
        ...
        <div class="flex items-center gap-3">
          <button on:click={onOpenCreate}
            ...
            <span class="text-sm font-medium tracking-wide truncate max-w-[70px] hidden md:block">{m.lobby_btn_open_create_char()}</span>
          </button>
        
          <button on:click={onOpenSettings} aria-label={m.lobby_btn_open_settings()}
      ...
      {#if viewMode === 'grid'}
        <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(300px, 1fr)); gap:1rem;">
          {#each filtered as char (char.id)}
            <div class="group relative w-full bg-white/[0.03] hover:bg-white/[0.07] border border-white/[0.06] hover:border-ryokan-accent/40 rounded-2xl overflow-hidden transition-all duration-200 active:scale-[0.98]">
              ...
              <div class="relative z-10 w-full aspect-[3/4] bg-white/5 overflow-hidden pointer-events-none">
                ...
                {/if}
                <div class="absolute top-2.5 right-2.5 z-20 pointer-events-auto opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0">
      ...
      {:else if viewMode === 'compact'}
        <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(150px, 1fr)); gap:0.75rem;">
          {#each filtered as char (char.id)}
            <div class="group relative w-full bg-white/[0.03] hover:bg-white/[0.07] border border-white/[0.06] hover:border-ryokan-accent/40 rounded-xl overflow-hidden transition-all duration-200 active:scale-[0.98]">
              <button on:click={() => onSelectChar(char)} aria-label={m.lobby_aria_start_chat()} class="absolute inset-0 z-0 w-full h-full cursor-pointer"></button>
              <div class="relative z-10 w-full aspect-square bg-white/5 overflow-hidden pointer-events-none">
                ...
                {/if}
                <div class="absolute top-1.5 right-1.5 z-20 pointer-events-auto opacity-0 group-hover:opacity-100 transition-all duration-200">
                  <button on:click={(e) => onEditChar(e, char)} aria-label={m.lobby_aria_edit_char()} title={m.lobby_aria_edit_char()}
      ...
      {:else if viewMode === 'list'}
        <div class="flex flex-col gap-2">
          {#each filtered as char (char.id)}
            <div class="group relative w-full bg-white/[0.03] hover:bg-white/[0.07] border border-white/[0.06] hover:border-ryokan-accent/40 rounded-xl overflow-hidden transition-all duration-200 active:scale-[0.995]">
              <button on:click={() => onSelectChar(char)} aria-label={m.lobby_aria_start_chat()} class="absolute inset-0 z-0 w-full h-full cursor-pointer"></button>
              <div class="relative z-10 flex items-center gap-4 px-4 py-3 pointer-events-none">
                ...
                <div class="pointer-events-auto opacity-0 group-hover:opacity-100 transition-opacity duration-150 shrink-0">
                  <button on:click={(e) => onEditChar(e, char)} aria-label={m.lobby_aria_edit_char()} title={m.lobby_aria_edit_char()}
      ...
      {#if filtered.length === 0}
        <div class="flex flex-col items-center justify-center py-24 text-center">
          {#if searchQuery.trim() !== ''}
            ...
            <button on:click={() => (searchQuery = '')} class="mt-3 text-xs text-ryokan-accent hover:underline">{m.lobby_reset_search()}</button>
          {:else}
