...
<div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(150px, 1fr)); gap:0.75rem;">
  {#each characters as char (char.id)}
    <div class="group relative w-full bg-white/[0.03] hover:bg-white/[0.07] border border-white/[0.06] hover:border-ryokan-accent/40 rounded-xl overflow-hidden transition-all duration-200 active:scale-[0.98] {showHidden && characterState.hiddenCharacterIds.has(String(char.id)) ? 'opacity-40' : ''}">
      ...
      <div class="relative z-10 w-full aspect-square bg-white/5 overflow-hidden pointer-events-none">
        ...
        <div class="absolute top-1.5 right-1.5 z-20 pointer-events-auto transition-all duration-200 {showHidden && characterState.hiddenCharacterIds.has(String(char.id)) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}">
          {#if !char.isCustom}
            <button
