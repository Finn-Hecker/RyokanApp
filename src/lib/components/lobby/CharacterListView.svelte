...
<div class="flex flex-col gap-2">
  {#each characters as char (char.id)}
    <div class="group relative w-full bg-white/[0.03] hover:bg-white/[0.07] border border-white/[0.06] hover:border-ryokan-accent/40 rounded-xl overflow-hidden transition-all duration-200 active:scale-[0.995] {showHidden && characterState.hiddenCharacterIds.has(String(char.id)) ? 'opacity-40' : ''}">
      ...
      <div class="relative z-10 flex items-center gap-4 px-4 py-3 pointer-events-none">
        ...
        <div class="pointer-events-auto transition-opacity duration-150 shrink-0 {showHidden && characterState.hiddenCharacterIds.has(String(char.id)) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}">
          {#if !char.isCustom}
            <button
              onclick={(e) => onToggleHide(e, char)}
              aria-label={characterState.hiddenCharacterIds.has(String(char.id)) ? 'Show' : 'Hide'}
              title={characterState.hiddenCharacterIds.has(String(char.id)) ? 'Show' : 'Hide'}
